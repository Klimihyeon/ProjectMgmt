...
<div class="replyLi" >
	<div class="user-block">
		<img src="<%=request.getContextPath()%>/student/manage/mypage/getPictureById/{{cqbId}}"
...
window.onload=function(){
	...
	getPage("<%=request.getContextPath()%>/student/manage/board/qna/replies/${cqbBoard.cqbNo}/"+replyPage);
...
function replyRegist_go(){
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/student/manage/board/qna/replies",
		type:"post",
		...
		success:function(data){
			...
			if(result[0]=="SUCCESS"){
				...
				getPage("<%=request.getContextPath()%>/student/manage/board/qna/replies/"+cqbNo+"/"+result[1]); //리스트 출력
				$('#newReplyText').val("");				
			}else{
...
function replyModify_go(){
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/student/manage/board/qna/replies/"+cqbRno,
		type:"PUT",
		headers:{			
...
function replyRemove_go(cqbRno){
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/student/manage/board/qna/replies/${cqbBoard.cqbNo}/"+cqbRno+"/"+replyPage,
		type:"DELETE",
		headers:{
		...
		success:function(page){
			alert("삭제되었습니다.");
			getPage("<%=request.getContextPath()%>/student/manage/board/qna/replies/${cqbBoard.cqbNo}/"+page);
			replyPage=page;
		},
		error:function(error){
			AjaxErrorSecurityRedirectHandler(error.status);
