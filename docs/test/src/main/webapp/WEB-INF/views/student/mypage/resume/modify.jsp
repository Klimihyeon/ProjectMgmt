...
<div class="card">
	...
	<div class="card-body">
	<input style="width: 90%; float: left;" id="largeInput" name="resumeTitle" class="form-control form-control-lg" value="${resume.resumeTitle }" type="text" placeholder="이력서 제목을 입력해주세요.">
	<select style="width: 10%; float: left; height: 46px;" class="form-select" name="isopen">
		<option>공개여부</option>
		<option ${resume.isopen=='공개' ? 'selected':'' }>공개</option>
		<option ${resume.isopen=='비공개' ? 'selected':'' }>비공개</option>
	</select>
	<input type="text" name="resumeCode" value="${resume.resumeCode }" hidden="" >
	<input type="text" name="studentId" value="${loginUser.usersId }" hidden="">
	</div>
</div>
			...
			<div class="card-body">
				...
				<table id="eduTable" class="table">
					...
					<tbody>
						...
						<tr>
							<td>
							<input type="text" name="educationCode" value="${education.educationCode }" hidden="">
							<select id="edu" class="form-select"
								name="educationInfo">
									<option value="선택">선택</option>
									<option value="2,3년제 대학" ${education.educationInfo=='2,3년제 대학' ? 'selected':'' }>2,3년제 대학</option>
									<option value="4년제 대학" ${education.educationInfo=='4년제 대학' ? 'selected':'' }>4년제 대학</option>
							</select>
							<input id="edu1" class="form-control" name="educationDetail" value="${education.educationDetail }" type="text"  style="margin-top: 10px;">
								</td>
							<td style="text-align: center;">
								<input class="form-control" name="edStartMonth" value="<fmt:formatDate value="${education.edStartMonth }" pattern="yyyy-MM-dd" />"
								type="date" style="width: 40%; float: left;"> <span
								style="float: left;">&nbsp;&nbsp;~&nbsp;&nbsp;</span>
								<input class="form-control" name="edFinalMonth" type="date" value="<fmt:formatDate value="${education.edFinalMonth }" pattern="yyyy-MM-dd" />" style="width: 40%; float: left;">
							</td>
							<td>
								<input class="form-control" value="${education.major }" name="major" type="text">
							</td>
							<td style="text-align: center;">
								<div class="row" style="text-align: center;">
									<input class="form-control" value="${education.score }" name="score" type="text" style="width: 50px;">
									 &nbsp;/ &nbsp;
									<input class="form-control" value="${education.maxScore }" name="maxScore" type="text" style="width: 50px;">
								</div>
							</td>
							<td>
								<select id="edInfo" class="form-select"  name="edInfo" style="width: 100px;">
										<option value="졸업" ${education.edInfo=='졸업' ? 'selected':'' }>졸업</option>
										<option value="휴학" ${education.edInfo=='휴학' ? 'selected':'' }>휴학</option>
										<option value="중퇴" ${education.edInfo=='중퇴' ? 'selected':'' }>중퇴</option>
										<option value="재학" ${education.edInfo=='재학' ? 'selected':'' }>재학</option>
								</select>
							</td>
						</tr>
						</c:forEach>
			...
			<div class="table-responsive">
				<table id="carTable" class="table">
					...
					<tbody>
						...
						<tr>
							<td>
							<input type="text" name="careerCode" value="${career.careerCode }" hidden="">
							<input value="<fmt:formatDate value="${career.careerStartMonth }" pattern="yyyy-MM-dd" />"  class="form-control" name="careerStartMonth"
								type="date" style="width: 45%; float: left;"> <span
								style="float: left;">&nbsp;&nbsp;~&nbsp;&nbsp;</span> <input
								value="<fmt:formatDate value="${career.careerFinalMonth }" pattern="yyyy-MM-dd" />" class="form-control" name="careerFinalMonth" type="date"
								style="width: 45%; float: left;"></td>
							<td><input class="form-control" name="careerCompany"
								value="${career.careerCompany }" type="text"></td>
							<td><input class="form-control" name="careerDivision"
								value="${career.careerDivision }" type="text"></td>
							<td><select  class="form-select" name="careerInfo">
									<option value="선택" >선택</option>
									<option value="퇴직" ${career.careerInfo=='퇴직' ? 'selected':'' }>퇴직</option>
									<option value="재직" ${career.careerInfo=='재직' ? 'selected':'' }>재직</option>
							</select></td>
						</tr>
						<tr>
							<th>업무내용</th>
			...
			<div class="table-responsive">
				<table id="cerTable" class="table">
					...
					<tbody>
						...
						<tr>
							<td>
							<input type="text" name="certCode" value="${certification.certCode }" hidden="">
							<input value="${certification.certName }" class="form-control" name="certName" type="text"></td>
							<td><input value="<fmt:formatDate value="${certification.certDate }" pattern="yyyy-MM-dd" />" class="form-control" name="certDate" type="date"></td>
							<td><input value="${certification.certCom }" class="form-control" name="certCom" type="text"></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
			...
			<div class="col-sm-12">
				<div class="border rounded p-2">
					...
					<div class="fileInput mb-2">
						<ul>
							<c:forEach items="${resume.resAttachList }" var="resAttach" >
							<br>
							<li class="attach-item">			
									<a name="attachedFile"attach-fileName="${resAttach.fileName }" attach-no="${resAttach.resAno }" 
										href="<%=request.getContextPath()%>/student/manage/mypage/resume/getFile.do?resAno=${resAttach.resAno }">													
...
<!-- 등록버튼 -->
	<div id="divBtn" style="text-align: right;">
		<button type="button"
			...
			id="modifyBtn" onclick="modify_submit();">수정</button>
      ...
      //submit =============================================
        function modify_submit(){
        	...
        	if($("input[name='resumeTitle']").val()==""){
        		alert(input.name+"은 필수입니다.");
        		$("input[name='resumeTitle']").focus();
        		return;
        	}
