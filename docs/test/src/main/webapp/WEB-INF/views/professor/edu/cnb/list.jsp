...
<c:set var="cnbList" value="${dataMap.cnbList }" />
...
<body>
		...
		<div class="card">
			<div class="card-header">
				<h2 class="card-title">공지사항-${course.ingCourseName }</h2>
			</div>
  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header with-border">
					<button type="button" class="btn btn-primary" id="registBtn" onclick="OpenWindow('registForm.do?ingCourseCode=${course.ingCourseCode}','공지등록',800,700);">공지등록</button>				
        ...
        <table class="table table-hover">
          ...
          <tbody>
			...
			<c:forEach items="${cnbList }" var="cnb">
				<tr onclick="OpenWindow('detail.do?cnbNo=${cnb.cnbNo}','상세조회',800,700);">
					<td>${cnb.cnbNo }</td> 
					<td>${cnb.cnbTitle }</td> 
					<td>${cnb.cnbWriter }</td> 
					<td><fmt:formatDate value='${cnb.cnbDate }' pattern="yyyy-MM-dd" /></td> 
					<td>${cnb.cnbCnt }</td> 
				</tr>
			</c:forEach>
