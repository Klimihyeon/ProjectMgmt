		...
		<div class="input-group">
			<div class="col-md-1">
				<select class="form-control" name="searchType" id="searchType">
					...
					<option value="n" ${cri.searchType eq 'n' ? 'selected' : ''}>아이디</option>
					<option value="w" ${cri.searchType eq 'w' ? 'selected' : ''}>이름</option>
				</select>
			</div>
				...
				<table class="table">
						...
						<tbody>
							<c:forEach var="users" items="${usersList}" varStatus="vs">
								<tr>
									<td>${fn:length(usersList) - vs.index }</td>
									<td>${users.usersId }</td>
									<td>${users.facName }</td>
									<td>${users.facPhone}</td>
									<td>${users.facEmail}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
